...
# [0-9] deliberately, never \d: for a str pattern Python's \d matches every
# Unicode decimal digit, so "+1" followed by Arabic-Indic, Devanagari,
# fullwidth, or Burmese digits would pass an E.164 check written with \d and
# then be sent to the provider verbatim. E.164 is ASCII digits only, and the
# jurisdiction router slices this string by index (see
...
# Longest provider free-text string this app will ever render in one field.
# Not protocol limits - purely bounds on what can be dumped into an
# operator's terminal or redirected log. Applied per field, never to the
# assembled JSON: truncating the serialized document would produce
# invalid JSON, so each string is bounded on its own and the structure
# always stays parseable.
MAX_DISPLAY_CHARS = 2000
# Identifiers and short enum-like values (ids, statuses, codes, locales).
# A well-behaved provider sends a handful of characters here; anything
# longer is not information, it is payload.
MAX_IDENTIFIER_DISPLAY_CHARS = 128
# The task echoed back in the response. Larger than the general limit so
# a normal hardened task (~3.5k characters, see build_hardened_task)
# stays fully readable, but still bounded against a hostile echo.
MAX_TASK_DISPLAY_CHARS = 8000

# Per-field display limits, keyed by the JSON field name the string was
# found under. Anything absent from this map gets MAX_DISPLAY_CHARS.
DISPLAY_LIMITS = {
    "task": MAX_TASK_DISPLAY_CHARS,
    "id": MAX_IDENTIFIER_DISPLAY_CHARS,
    "object": MAX_IDENTIFIER_DISPLAY_CHARS,
    "status": MAX_IDENTIFIER_DISPLAY_CHARS,
    "failure_code": MAX_IDENTIFIER_DISPLAY_CHARS,
    "provider_call_id": MAX_IDENTIFIER_DISPLAY_CHARS,
    "locale": MAX_IDENTIFIER_DISPLAY_CHARS,
    "region": MAX_IDENTIFIER_DISPLAY_CHARS,
    "phones": MAX_IDENTIFIER_DISPLAY_CHARS,
    "phone": MAX_IDENTIFIER_DISPLAY_CHARS,
    "speaker": MAX_IDENTIFIER_DISPLAY_CHARS,
    "label": MAX_IDENTIFIER_DISPLAY_CHARS,
    "created_at": MAX_IDENTIFIER_DISPLAY_CHARS,
    "completed_at": MAX_IDENTIFIER_DISPLAY_CHARS,
    "started_at": MAX_IDENTIFIER_DISPLAY_CHARS,
}


def sanitize_for_display(value: Any, max_chars: int = MAX_DISPLAY_CHARS) -> str:
    """Render provider-controlled text safely for a terminal or a log.

    DISPLAY ONLY - never call this on data the engine reasons about.
    verdict.reconcile() must keep receiving CALL-E's structured_result
    exactly as received; sanitizing decision inputs would mean deciding
    on something other than what the provider actually said.

    Free text reaching this app (summary, failure_message, evidence,
    transcript turns, and the model-written confidence_note /
    manipulation_attempt_note) is not trustworthy input: the last two are
    generated by a model from a live conversation, so a counterpart
    practising prompt injection can influence them (see the README's
    "Prompt injection resistance" section). Raw ESC/control bytes in that
    text would otherwise be interpreted by the terminal rather than
    displayed.

    Every character outside printable ASCII plus space is replaced by its
    \\xNN / \\uNNNN escape, so nothing can move the cursor, recolour, clear
    the screen, or hide text. Length is bounded, with the truncation
    ...
    if len(escaped) <= max_chars:
        return escaped
    # The marker is counted inside max_chars, never added on top of it, so
    # the result is always <= max_chars and re-sanitizing an already
    # sanitized string is a no-op (idempotence). Truncating to max_chars
    # and then appending would grow past the limit and would truncate
    # again on a second pass.
    marker = f"...[truncated, {len(escaped)} chars total]"
    if len(marker) >= max_chars:
        # Limit too small to hold even the marker. Return the marker's own
        # head rather than silently exceeding the bound: truncation stays
        # visible and the guarantee len(result) <= max_chars still holds.
...
class CallEAPIError(Exception):
    ...
    message/details keep the provider's raw values (callers may need
    them); only the rendered str() of this exception is sanitized, since
    that is what reaches a terminal.
    """
    ...
    def __init__(self, status_code: int, code: str, message: str, details: dict[str, Any]) -> None:
        ...
        text = (
            f"CALL-E API error {sanitize_for_display(code)} "
            f"(HTTP {status_code}): {sanitize_for_display(message)}"
        )
        if details:
            text = f"{text}\nDetails: {sanitize_for_display(details)}"
        if hint:
            text = f"{text}\nHint: {hint}"
        super().__init__(text)
...
def _sanitized_copy(value: Any, key: str | None = None) -> Any:
    """Recursively sanitize every string in a nested structure, applying
    the per-field limit for the key that string was found under.

    Lists inherit their parent key, so evidence[] entries get evidence's
    limit and phones[] get phones'. Non-string scalars (numbers, bools,
    null) are left alone: they are not free text, and json.dumps renders
    them without any escaping question.
    """
    if isinstance(value, dict):
        return {inner_key: _sanitized_copy(inner, inner_key) for inner_key, inner in value.items()}
    if isinstance(value, list):
        return [_sanitized_copy(item, key) for item in value]
    if isinstance(value, str):
        return sanitize_for_display(value, DISPLAY_LIMITS.get(key, MAX_DISPLAY_CHARS))
    return value


def redacted_call_for_display(call: dict[str, Any]) -> dict[str, Any]:
    """Display-only copy of a CallTask response: phone numbers masked,
    then every string field sanitized and length-bounded per field.

    This is the single central point for rendering a provider response.
    Order matters: masking runs first (so a hostile phone value is
    reduced to its last 4 characters before anything else), then
    _sanitized_copy escapes control characters and bounds each field.
    json.dumps already escapes control characters on its own, but only
    while ensure_ascii stays at its default and never with any length
    bound at all - a multi-megabyte summary would still be dumped whole.
    Doing it here means the guarantee holds regardless of how the caller
    serializes, and each field is bounded independently so the JSON
    structure itself is never truncated and stays parseable.

    DISPLAY ONLY. The caller keeps the original response object for the
    engine: verdict.reconcile() must see CALL-E's structured_result
    exactly as received, never this copy.
    """
    ...
    return _sanitized_copy(display)
...
# other fixed blocks. Real calls showed the agent treat a short,
# unclear, or interrupting reply as a cue to restart its opening from
# scratch rather than continue the conversation.
...
# principle as the other fixed blocks. A real call showed the agent end
# the call right after a bare, brief acknowledgement, with no recap,
# cutting the recipient off mid-reply - CALL-E has no real-time control
# this app can exercise over call flow, so the only lever is
# instructing the agent directly.
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            try:
                ...
                if retryable and exc.code in RETRYABLE_STATUS_CODES and attempt < MAX_ATTEMPTS:
                    ...
                    # code comes straight from the provider's error body,
                    # so it goes through the display sanitizer like any
                    # other provider free text before reaching a terminal.
                    print(
                        f"   retryable ({sanitize_for_display(code, MAX_IDENTIFIER_DISPLAY_CHARS)}), "
                ...
                # fields, alert descriptions), so it is not fully ours.
                print(f"<- connection error: {sanitize_for_display(exc.reason)}", flush=True)
                if not retryable:
